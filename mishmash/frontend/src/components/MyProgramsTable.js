...
const MyProgramsTable = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        const relevantApplications = programsWithStatus.filter(app => 
          app !== null && 
          app.status && 
          ['enrolled', 'applied', 'withdrawn', 'canceled'].includes(app.status.toLowerCase())
        );
  ...
  const compareValues = (a, b, orderBy) => {
    ...
    if (orderBy === 'status') {
      ...
      const statusOrder = { 'Enrolled': 4, 'Applied': 3, 'Withdrawn': 2, 'Canceled': 1 };
      return statusOrder[valueA] - statusOrder[valueB];
    }

    // Handle date fields
    if (orderBy === 'application_deadline' || 
        orderBy === 'start_date' || 
        orderBy === 'end_date') {
          ...
          {sortApplications(applications).map((application) => (
            <React.Fragment key={application.id}>
              ...
              >
                ...
                <StyledTableCell>{application.program.year_semester}</StyledTableCell>
                <StyledTableCell>{application.program.faculty_leads}</StyledTableCell>
                <StyledTableCell>{formatDate(application.program.application_deadline)}</StyledTableCell>
                <StyledTableCell>{formatDate(application.program.start_date)}</StyledTableCell>
                <StyledTableCell>{formatDate(application.program.end_date)}</StyledTableCell>
                    ...
                    <Box sx={{ margin: 2 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <ApplicationButton
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/apply/${application.program.id}/view`);
                          }}
                        >
                          View Application Details
                        </ApplicationButton>
                        {['withdrawn', 'canceled'].includes(application.status.toLowerCase()) &&
