...
const StatusBadge = styled('div')(({ theme, status }) => {
  const getColors = () => {
    switch (status?.toLowerCase()) {
      case 'enrolled':
      case 'applied':
        return { bg: theme.palette.status.info.background, color: theme.palette.status.info.main };
      case 'withdrawn':
      case 'canceled':
      case 'closed':
        return { bg: theme.palette.status.error.background, color: theme.palette.status.error.main };
      case 'opening soon':
        return { bg: theme.palette.status.warning.background, color: theme.palette.status.warning.main };
      case 'open':
        return { bg: theme.palette.status.success.background, color: theme.palette.status.success.main };
      default:
        return { bg: theme.palette.status.neutral.background, color: theme.palette.status.neutral.main };
    }
  };
...
const ApplicationStatusBadge = styled('div')(({ theme, status }) => {
  const getColors = () => {
    switch (status?.toLowerCase()) {
      ...
      case 'withdrawn':
      case 'canceled':
        return { bg: theme.palette.status.error.background, color: theme.palette.status.warning.main };
      default:
        return { bg: theme.palette.status.neutral.background, color: theme.palette.status.neutral.main };
    }
  };
...
const ApplicationButton = styled('button')(({ theme, variant }) => {
  const getColors = () => {
    switch (variant) {
      case 'success':
        return { bg: theme.palette.status.success.main, color: theme.palette.status.success.contrastText };
      case 'disabled':
        return { bg: theme.palette.status.neutral.light, color: theme.palette.status.neutral.contrastText };
      default:
...
const ProgramCard = ({ program, isInAppliedSection, onExpand }) => {
  ...
  const getDetailedStatus = () => {
    ...
    switch (applicationStatus.toLowerCase()) {
      ...
      case 'withdrawn': return 'Previously withdrawn';
      case 'canceled': return 'Previously canceled';
      default: return null;
    }
  };
  ...
  const getApplicationStatusText = () => {
    ...
    switch (applicationStatus.toLowerCase()) {
      ...
      case 'canceled': return 'Application Canceled';
      default: return null;
    }
  };
  ...
  const getApplicationButton = () => {
    ...
    // Enrolled/Applied
    if (status && ['enrolled', 'applied'].includes(status)) {
      return (
        <div style={{
          padding: '8px 16px',
          borderRadius: '4px',
    ...
    if (status && ['withdrawn', 'canceled'].includes(status)) {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
          {programStatus === 'Open' && (
            <ApplicationButton
              onClick={(e) => { e.stopPropagation(); navigate(`/apply/${program.id}`); }}
              variant="success"
            >
              Apply Again
            </ApplicationButton>
  ...
  function renderDeadlineIndicator() {
    if (applicationStatus && ['enrolled', 'applied'].includes(applicationStatus.toLowerCase())) {
