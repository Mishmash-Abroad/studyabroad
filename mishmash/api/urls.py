...
- /auth/token/:
    - GET: Return the DRF token for the current session (creates one if needed)

- /users/current_user/:
    - GET: Return the current user's details
...
from .views import *
from .auth_views import auth_token_view, current_user_view
...
router.register(r'notes', ConfidentialNoteViewSet, basename="confidentialnote")
...
urlpatterns = [
    ...
    path("auth/token/", auth_token_view, name="auth-token"),
    path("users/current_user/", current_user_view, name="current-user"),
    # Include allauth URLs for SSO and MFA
    path('accounts/', include('allauth.urls')),
    path('accounts/mfa/', include('allauth.mfa.urls')),  # Include MFA URLs
]
